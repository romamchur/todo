              ...
              if(this.tasks[i] == task){
                ...
                for(let i=0;i<tempTeams.length;i++){
                  for(let j=0;j<tempTeams[i].tasks.length;j++){
                    ...
                    if(task.name==tempTeams[i].tasks[j].name && task.team.value == tempTeams[i].tasks[j].team.value){
                      var indexTeam = i;
                      var indexTask = j;
                      break;
                    }
                ...
                firebase.database().ref('users/'+id+'/teams'+'/team').child(indexTeam).child('/tasks').child(indexTask).child('main').set({
                  main:false
                })
              }
            }
          ...
          console.log(snapshot.val())
            for(let i=0;i<snapshot.val().length;i++) {
              for (let j = 0; j < snapshot.val()[i].tasks.length;j++) {
                if (snapshot.val()[i].tasks[j].main.main==true) {
